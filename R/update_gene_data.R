...
#'
update_gene_data <- function(server,
                             database,
                             user,
                             update = FALSE) {
  # Fetch password
  ...
  RESULT_gene_data <- init_table %>%
    ...
    filter(!is.na(gene)) %>%
    select(metodekode_sens, gene_desc, gene_method, analyttkode, analyttkode_sens, gene) %>%
    distinct()

  only_in_old <- anti_join(old_table, RESULT_gene_data, by = names(old_table))
  only_in_new <- anti_join(RESULT_gene_data, old_table, by = names(RESULT_gene_data))
